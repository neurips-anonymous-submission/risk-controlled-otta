...
def build_source_augmentation() -> A.Compose:
    """
    Paper-aligned source-domain photometric augmentation.

    The operations are implemented with Albumentations and each is applied
    independently with probability 0.5:
    sunflare, histogram equalization, blur, contrast, and noise.
    """
    return A.Compose([
...
def apply_source_augmentation(image: np.ndarray, augmentation: A.Compose | None = None) -> np.ndarray:
    if augmentation is None:
        augmentation = build_source_augmentation()
...
class SpeedPlusCropHeatmapDataset(Dataset):
    def __init__(
        ...
        self.source_augmentation = build_source_augmentation() if use_augmentation else None
    ...
    def __getitem__(self, index: int) -> Dict[str, torch.Tensor]:
        ...
        if self.use_augmentation:
            cropped_image = apply_source_augmentation(cropped_image, self.source_augmentation)
