param(
    ...
    [ValidateSet("strict_ltta", "strict_petta", "strict_hybrid")]
    [string]$Method,
    ...
    [string[]]$Streams = @("forward", "reverse", "cyclic", "shifted")
)
...
if (-not $OutputDir) { $OutputDir = Join-Path $RepoRoot ("output\mixed_domain_stream_results_strict\{0}" -f $Method.Replace("strict_", "")) }
...
switch ($Method) {
    "strict_ltta" {
        & $PythonExe -m risk_controlled_otta.experiments.strict_mixed_domain_stream_eval `
            --data_root $DataRoot `
