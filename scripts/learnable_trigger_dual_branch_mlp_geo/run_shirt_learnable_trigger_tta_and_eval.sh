...
TTA_OUTPUT_DIR="output/dinov3_heatmap_shirt_dual_branch_tta_v4_rot_safe_relaxed"
TTA_MODEL_PATH="${TTA_OUTPUT_DIR}/tta_final.pth"
EVAL_OUTPUT_DIR="${TTA_OUTPUT_DIR}/eval_after_tta"

echo "[1/2] Running SHIRT rotation-safe dual-branch OTTA (relaxed)..."
python risk-controlled-otta/adapt/learnable_trigger_single_model_tta_shirt_dual_branch_v3_safe.py \
  ...
  --memory_min_quality 0.24 \
  --memory_min_confidence 0.54 \
  --memory_min_inliers 7 \
  --memory_min_inlier_ratio 0.62 \
  --memory_max_reproj_error 5.0 \
  --memory_min_tvec_norm 4.3 \
  --memory_max_tvec_norm 8.8 \
  --memory_peak_conf_thresh 0.38 \
  --memory_concentration_thresh 0.14 \
  --memory_update_margin 0.01 \
  --teacher_sharpen_temperature 0.88 \
  --lambda_self_training 0.22 \
  --lambda_geo 0.12 \
  --lambda_reg 0.04 \
  --tau 0.7 \
  --grad_clip_norm 1.0 \
  --geo_boost_min_inliers 7 \
  --geo_boost_min_inlier_ratio 0.68 \
  --geo_boost_max_reproj 5.0 \
  --trigger_confidence 0.20 \
  --trigger_min_inliers 6 \
  --trigger_min_inlier_ratio 0.45 \
  --trigger_reprojection_error 6.5 \
  --trigger_min_tvec_norm 4.3 \
  --trigger_max_tvec_norm 8.8 \
  --geometry_min_confidence 0.54 \
  --geometry_min_inliers 7 \
  --geometry_min_inlier_ratio 0.68 \
  --geometry_max_reproj_error 4.6 \
  --geometry_min_tvec_norm 4.3 \
  --geometry_max_tvec_norm 8.8 \
