...
python risk-controlled-otta/adapt/learnable_trigger_single_model_tta_dino_heatmap_dual_branch_v2.py \
  ...
  --output_dir output/dinov3_heatmap_dual_branch_sunlamp_v2 \
...
python risk-controlled-otta/eval/evaluate_dino_heatmap.py \
  --data_root speedplusv2 \
  --model_path output/dinov3_heatmap_dual_branch_sunlamp_v2/tta_final.pth \
  --split sunlamp \
  --output_dir evaluation_results_dinov3_heatmap/dual_branch_sunlamp_v2 \
  --num_vis 20 \
  --nms_kernel 3 \
  --subpixel_radius 2 \
  --min_confidence 0.05 \
