...
python risk-controlled-otta/adapt/triggered_single_model_tta_dino_heatmap.py \
  ...
  --output_dir output/dinov3_heatmap_triggered_lightbox_v1 \
...
python risk-controlled-otta/eval/evaluate_dino_heatmap.py \
  --data_root speedplusv2 \
  --model_path output/dinov3_heatmap_triggered_lightbox_v1/tta_final.pth \
  --split lightbox \
  --output_dir evaluation_results_dinov3_heatmap/triggered_lightbox_v1 \
  --num_vis 20 \
  --nms_kernel 3 \
  --subpixel_radius 2 \
  --min_confidence 0.05 \
