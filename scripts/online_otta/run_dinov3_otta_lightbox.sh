...
python risk-controlled-otta/adapt/online_tta_dino_heatmap_geom.py \
  ...
  --output_dir output/dinov3_heatmap_tta_lightbox_geom_v1 \
...
python risk-controlled-otta/eval/evaluate_dino_heatmap.py \
  --data_root speedplusv2 \
  --model_path output/dinov3_heatmap_tta_lightbox_geom_v1/tta_final.pth \
  --split lightbox \
  --output_dir evaluation_results_dinov3_heatmap/otta_lightbox_geom_v1 \
  --num_vis 20 \
  --nms_kernel 3 \
  --subpixel_radius 2 \
  --min_confidence 0.05 \
