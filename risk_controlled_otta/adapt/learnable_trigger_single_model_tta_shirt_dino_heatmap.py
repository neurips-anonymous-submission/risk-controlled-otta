...
class ShirtTTADataset(Dataset):
    """SHIRT crop dataset for test-time adaptation.

    The dataset uses the provided pose annotations only to reproduce the same object crop used by
    training/evaluation. The TTA losses below do not consume ground-truth heatmaps or poses.
    """
...
class FeatureQualityMemoryBank:
    ...
    def push(self, image: torch.Tensor, pseudo_heatmap: torch.Tensor, feature: torch.Tensor, quality: float, image_name: str) -> None:
    ...
    def sample(self, batch_size: int, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, np.ndarray]:
        ...
        images = torch.stack([self.entries[int(i)].image for i in indices], dim=0).to(device)
        pseudo = torch.stack([self.entries[int(i)].pseudo_heatmap for i in indices], dim=0).to(device)
        weights = torch.tensor([self.entries[int(i)].quality for i in indices], dtype=torch.float32, device=device)
        weights = weights / weights.mean().clamp_min(1e-6)
        return images, pseudo, weights, indices
...
class GeoRiskMLP(nn.Module):
    def __init__(self, input_dim: int = 8, hidden_dim: int = 16) -> None:
        super().__init__()
        self.net = nn.Sequential(nn.Linear(input_dim, hidden_dim), nn.ReLU(inplace=True), nn.Linear(hidden_dim, 1))
...
class DualBranchRiskGate(nn.Module):
    def __init__(self, geo_dim: int = 8, feat_dim: int = 3, hidden_dim: int = 16) -> None:
        super().__init__()
        self.geo_branch = nn.Sequential(nn.Linear(geo_dim, hidden_dim), nn.ReLU(inplace=True), nn.Linear(hidden_dim, 1))
        self.feat_branch = nn.Sequential(nn.Linear(feat_dim, max(4, hidden_dim // 2)), nn.ReLU(inplace=True), nn.Linear(max(4, hidden_dim // 2), 1))
        self.fusion = nn.Linear(2, 1)

    def forward(self, geo_features: torch.Tensor, feat_features: torch.Tensor) -> torch.Tensor:
        return self.fusion(torch.cat([self.geo_branch(geo_features), self.feat_branch(feat_features)], dim=-1)).squeeze(-1)
...
def heuristic_risk_label(diagnosis: Dict[str, object], args, device: torch.device) -> torch.Tensor:
    ...
    label = mean_conf < args.trigger_confidence or num_inliers < args.trigger_min_inliers or mean_reproj > args.trigger_reprojection_error or fallback or pnp_failed
...
def build_source_prototype(model: DinoHeatmapPoseModel, args, device: torch.device) -> torch.Tensor | None:
    if args.trigger_mode != "dual_branch" or args.prototype_max_samples == 0:
        return None
    dataset = ShirtTTADataset(
        data_root=args.data_root,
        roe=args.roe,
        domain=args.prototype_domain,
    ...
    loader = DataLoader(dataset, batch_size=args.prototype_batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=(device.type == "cuda"))
    features: List[torch.Tensor] = []
    seen = 0
    model.eval()
    for batch in tqdm(loader, desc="build_shirt_source_prototype", leave=False):
    ...
    if not features:
        return None
    return F.normalize(torch.cat(features, dim=0).mean(dim=0).flatten(), dim=0).cpu()


def gate_probability(gate: nn.Module | None, geo_features: torch.Tensor, feat_features: torch.Tensor, args) -> torch.Tensor:
...
def scale_optimizer_lr(optimizer: AdamW, scale: float) -> List[float]:
...
def restore_optimizer_lr(optimizer: AdamW, old_lrs: List[float]) -> None:
    for group, old_lr in zip(optimizer.param_groups, old_lrs):
        group["lr"] = old_lr
...
def adapt_with_gate(
    ...
    for _ in range(args.adapt_steps):
        ...
        old_lrs = scale_optimizer_lr(optimizer, max(gate_weight, args.min_lr_gate_scale)) if args.gate_usage == "soft_lr" else None
...
def run_learnable_trigger_tta(args) -> None:
    ...
    for step, batch in enumerate(tqdm(target_loader, desc=f"shirt_learnable_trigger_{args.trigger_mode}"), start=1):
        ...
        diagnosis = diagnose_prediction(heatmap, bbox, args.input_size, camera_matrix, dist_coeffs, args)
        ...
        if float(diagnosis["quality"]) >= args.memory_min_quality:
            memory_bank.push(image.squeeze(0), heatmap.squeeze(0), cls_token.squeeze(0), float(diagnosis["quality"]), image_name)
        ...
        if triggered and len(memory_bank) >= args.min_memory_for_update:
            ...
            losses = adapt_with_gate(model, optimizer, scaler, memory_bank, image, geometry_target, gate_weight, args, device)
    ...
    with (output_dir / "shirt_learnable_trigger_history.json").open("w", encoding="utf-8") as handle:
        json.dump({"summary": summary, "history": history, "loss_history": loss_history, "gate_history": gate_history}, handle, indent=2)
    ...
    checkpoint = {
        ...
        "dataset": "SHIRT",
        "adaptation": "learnable_trigger_single_model_tta_shirt",
        "trigger_mode": args.trigger_mode,
        "gate_usage": args.gate_usage,
        "update_scope": args.update_scope,
        "summary": summary,
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="output/dinov3_heatmap_shirt_learnable_trigger_tta")
    ...
    parser.add_argument("--prototype_domain", type=str, default="synthetic")
    parser.add_argument("--prototype_split", type=str, choices=["train", "val", "all"], default="train")
