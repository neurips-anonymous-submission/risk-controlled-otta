...
def diagnose_prediction(
    ...
    quality = float(max(0.0, 0.45 * conf_term + 0.30 * inlier_term + 0.25 * reproj_term))
...
def filter_heatmap_by_peak_confidence(heatmap: torch.Tensor, peak_thresh: float) -> torch.Tensor:
    if heatmap.ndim != 3:
        raise ValueError(f"Expected [K,H,W] heatmap, got shape {tuple(heatmap.shape)}")
    peaks = heatmap.flatten(1).amax(dim=1)
    keep = (peaks >= peak_thresh).to(dtype=heatmap.dtype).view(-1, 1, 1)
    return heatmap * keep


def sharpen_heatmap(heatmap: torch.Tensor, temperature: float = 0.7, eps: float = 1e-8) -> torch.Tensor:
    if temperature <= 0:
        return heatmap
...
def choose_gate_decision(step: int, heuristic_label: torch.Tensor, risk_prob: torch.Tensor, args) -> Tuple[bool, float]:
    # label/prob 表示 risk，高风险不更新，低风险才更新。
...
def adapt_with_gate(
    ...
    geo_weight_eff = args.lambda_geo * compute_geo_boost(diagnosis, args)
    ...
    with torch.no_grad():
        updated_mem_pred = teacher_model(mem_images)
        updated_quality = updated_mem_pred.detach().flatten(2).amax(dim=-1).mean(dim=1)
        filtered_pred = torch.stack(
            [
                filter_heatmap_by_peak_confidence(updated_mem_pred[i], args.memory_peak_conf_thresh)
                for i in range(updated_mem_pred.shape[0])
            ],
            dim=0,
        )
        memory_bank.update_if_better(mem_indices, filtered_pred, updated_quality)
...
def build_teacher_memory_target(teacher_heatmap: torch.Tensor, args) -> torch.Tensor:
    filtered = filter_heatmap_by_peak_confidence(teacher_heatmap, args.memory_peak_conf_thresh)
...
def run_learnable_trigger_tta(args) -> None:
    ...
    for step, batch in enumerate(tqdm(target_loader, desc="learnable_trigger_shirt_dual_branch_v3_safe"), start=1):
    ...
    checkpoint = {
        ...
        "roe": args.roe,
        "domain": args.domain,
        "adaptation": "learnable_trigger_single_model_tta_shirt_dual_branch_v3_safe",
        "trigger_mode": "dual_branch_safe",
        "gate_usage": args.gate_usage,
        "update_scope": args.update_scope,
        "summary": summary,
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="output/dinov3_heatmap_shirt_dual_branch_tta_v3_safe")
    ...
    parser.add_argument("--memory_min_quality", type=float, default=0.30)
    parser.add_argument("--memory_min_confidence", type=float, default=0.55)
    parser.add_argument("--memory_min_inliers", type=int, default=7)
    parser.add_argument("--memory_max_reproj_error", type=float, default=5.0)
    ...
    parser.add_argument("--memory_peak_conf_thresh", type=float, default=0.35)
    parser.add_argument("--teacher_sharpen_temperature", type=float, default=0.85)

    parser.add_argument("--lambda_self_training", type=float, default=0.20)
    parser.add_argument("--lambda_geo", type=float, default=0.15)
    parser.add_argument("--lambda_reg", type=float, default=0.05)
    parser.add_argument("--tau", type=float, default=0.7)
    parser.add_argument("--grad_clip_norm", type=float, default=1.0)

    parser.add_argument("--geo_boost_min_inliers", type=int, default=7)
    parser.add_argument("--geo_boost_max_reproj", type=float, default=5.0)
    ...
    parser.add_argument("--geometry_min_confidence", type=float, default=0.55)
    parser.add_argument("--geometry_min_inliers", type=int, default=7)
    parser.add_argument("--geometry_max_reproj_error", type=float, default=4.5)
