...
def make_source_dataset(args, split: str, use_source_augmentation: bool = False) -> SpeedPlusDinoHeatmapDataset:
...
def online_adapt(args) -> None:
    ...
    source_dataset = make_source_dataset(args, "train", use_source_augmentation=False)
    target_dataset = TargetTTADataset(args.data_root, args.target_split, args.input_size)
    ...
    for step, batch in enumerate(tqdm(target_loader, desc="online_tta_dino_geom"), start=1):
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="output/dinov3_heatmap_tta_geom")
