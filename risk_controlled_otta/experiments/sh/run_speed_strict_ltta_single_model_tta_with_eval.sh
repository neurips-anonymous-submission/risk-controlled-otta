...
LTTA_OUTPUT_DIR="${LTTA_OUTPUT_DIR:-output/dinov3_heatmap_speed_strict_ltta_single_model_tta}"
LTTA_SCRIPT="${LTTA_SCRIPT:-ltta_strict_single_model_tta_speed_dino_heatmap.py}"

TARGET_SPLIT="${TARGET_SPLIT:-lightbox}"
EVAL_SPLIT="${EVAL_SPLIT:-${TARGET_SPLIT}}"
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-${LTTA_OUTPUT_DIR}/eval_after_strict_ltta}"
SEED="${SEED:-42}"
MAX_SAMPLES="${MAX_SAMPLES:-0}"
NUM_VIS="${NUM_VIS:-20}"
COLLAPSE_THRESHOLD="${COLLAPSE_THRESHOLD:-0.1}"

mkdir -p "${LTTA_OUTPUT_DIR}" "${EVAL_OUTPUT_DIR}"

if [[ ! -f "${LTTA_SCRIPT}" ]]; then
  echo "[ERROR] Strict L-TTA script not found: ${LTTA_SCRIPT}" >&2
  echo "        Copy ltta_strict_single_model_tta_speed_dino_heatmap.py to output/risk_controlled_otta/experiments/" >&2
  echo "        or set LTTA_SCRIPT=/path/to/ltta_strict_single_model_tta_speed_dino_heatmap.py" >&2
  exit 1
fi
...
echo "[1/2] Running STRICT SPEED L-TTA single-model adaptation"
...
echo "[2/2] Evaluating STRICT SPEED L-TTA-adapted model"
python -m risk_controlled_otta.eval.evaluate_dino_heatmap \
  --data_root "${DATA_ROOT}" \
  --model_path "${LTTA_OUTPUT_DIR}/strict_ltta_final.pth" \
  --split "${EVAL_SPLIT}" \
...
echo "[OK] Strict L-TTA adaptation and evaluation finished."
echo "History: ${LTTA_OUTPUT_DIR}/strict_ltta_history.json"
echo "Final checkpoint: ${LTTA_OUTPUT_DIR}/strict_ltta_final.pth"
